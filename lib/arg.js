...
exports.validate = (argv) => {
  ...
  function isFileBody(val) { // does this string look like a file URL?
    return (val.startsWith('file://'));
  }

  function file(val) { // if the value is a file:// reads the file from disk, otherwise returns the value
      return isFileBody(val) ? fs.readFileSync(val.slice(7), {encoding: 'utf8'}) : val;
  }
  ...
  if (has('parameters')) {
    try {
      ...
      // if we have a params file then use it.
      //console.log(err);
      if (isFileBody(argv.parameters[0])) {
